...
class CatModel extends Model{
	...
	public function getCatsArr(){
		...
		$tree=[];
		// var_dump($tree);die;
		$this->recursiveCatArr($tree,$categorys);
		return $tree;
	}
	/**
	 * 递归整理分类数据
	 ...
	 * @param $dir_tree 表示整理之后的分类数据前的分类图标个数,顶级分类的值为0
	 */
	protected function recursiveCatArr(&$tree,$cats,$parent_id=0,$dir_tree=0){
		foreach($cats as $cat){//当前遍历到的这条分类的子分类
			if($cat['parent_id']==$parent_id){
				$cat['dir_tree']=$dir_tree;
				$tree[$parent_id][]=$cat;
				$this->recursiveCatArr($tree,$cats,$cat['id'],$dir_tree+1);
			}
		}
	}
